...
const CONTRACT_ADDRESS = "0xeFbc534AeaC9B2546e96E2fA51C5eDEC90CaAcF9"; // Your SafeSend contract address
...
export const WalletProvider: React.FC<WalletProviderProps> = ({ children }) => {
	...
	const createTransaction = async (
		...
		try {
			...
			try {
				...
				throw new Error(
					"Cannot connect to blockchain network. Please check your connection."
				);
			}
			...
			try {
				...
				if (contractCode === "0x" || contractCode.length <= 2) {
					...
					if (networkInfo.chainId === 1337n || networkInfo.chainId === 5777n) {
						throw new Error(
							`No SafeSend contract deployed at ${checksummedContractAddress} on your local Ganache network (Chain ID: ${networkInfo.chainId}). Please:\n1. Make sure Ganache is running\n2. Deploy your SafeSend contract to Ganache\n3. Update the contract address in the frontend\n4. Ensure the contract address matches exactly`
						);
					} else {
						throw new Error(
							`No contract found at address ${checksummedContractAddress} on network ${networkInfo.chainId}. Please verify the SafeSend contract is deployed to the correct network.`
						);
					}
				}
				...
				throw new Error(
					"Failed to verify contract deployment - network connectivity issue"
				);
			}
			...
			try {
				...
				console.log(
					"✓ Contract interaction successful - current transaction counter:",
					txCounter.toString()
				);
				...
				if (interactionError.code === "CALL_EXCEPTION") {
					throw new Error(
						"Contract exists but is not responding correctly. The contract may not be the expected SafeSend contract or may have compilation issues."
					);
				} else if (interactionError.message?.includes("network")) {
					throw new Error(
						"Network error when trying to interact with contract. Please check your Ganache connection."
					);
				} else {
					throw new Error(
						`Cannot communicate with smart contract: ${interactionError.message}`
					);
				}
			}
			...
			try {
				...
				const gasEstimate = await contract.createTransaction.estimateGas(
					validRecipient,
					codeHash,
					{
						value: amountWei,
				...
				if (gasError.reason) {
					...
					throw new Error(
						"Gas estimation failed - transaction may not be valid"
					);
				}
			}
			...
			if (error.code === "NETWORK_ERROR") {
				...
				throw new Error(
					"Smart contract call failed. Please verify the contract is deployed correctly."
				);
			} else if (error.code === "INSUFFICIENT_FUNDS") {
				throw new Error("Insufficient funds for transaction and gas fees.");
			} else if (error.code === "UNPREDICTABLE_GAS_LIMIT") {
				throw new Error(
					"Cannot estimate gas limit. Transaction may fail or contract may not be deployed."
				);
			} else if (error.message?.includes("user rejected")) {
				throw new Error("Transaction rejected by user");
			} else if (error.message?.includes("network")) {
				throw new Error(
					"Network error. Please check your connection to the blockchain network."
				);
			} else if (error.message?.includes("nonce")) {
				throw new Error(
					"Transaction nonce error. Please reset your MetaMask account."
				);
			} else if (error.reason) {
				throw new Error(error.reason);
			} else {
				throw new Error(
					error.message || "Unknown error occurred during transaction creation"
				);
			}
		}
	};
	...
	const claimFunds = async (txId: string, passcode: string) => {
		...
		try {
			...
			// Debug log: recipient
			const recipientOnChain = await contract
				.transactions(parsedTxId)
				.then((tx: any) => tx.recipient);
			...
			if (account.toLowerCase() !== recipientOnChain.toLowerCase()) {
				...
				throw new Error(
					"Your wallet is not the intended recipient. Please switch to the correct MetaMask account."
				);
			}
			...
			if (error.reason) {
				...
				if (error.message.toLowerCase().includes("invalid code")) {
					...
					error.message.toLowerCase().includes("not the intended recipient")
				) {
					throw new Error(
						"Only the intended recipient can claim this transaction (please switch to the recipient's MetaMask account)"
					);
				} else if (
					error.message.toLowerCase().includes("invalid number value")
				) {
					...
					throw new Error(
						"Your wallet is not the recipient for this transaction. Please login with the recipient address."
					);
				} else {
	...
	const getTransaction = async (txId: string) => {
		...
		try {
			...
			throw new Error(
				"Could not fetch transaction. Please check the ID and network."
			);
		}
	};
